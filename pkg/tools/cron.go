...
// SetContext stores a fallback delivery context when the tool is called
// without a tool-enriched execution context.
...
func (t *CronTool) resolveNotificationTargets(sourceChannel, sourceChatID string, broadcastAll bool) []cron.CronTarget {
	...
	appendConfigTargets(ch.Matrix.Enabled && strings.TrimSpace(ch.Matrix.Homeserver) != "", "matrix", ch.Matrix.AllowFrom)
	...
	appendConfigTargets(ch.WeComAIBot.Enabled && strings.TrimSpace(ch.WeComAIBot.Token) != "", "wecom_aibot", ch.WeComAIBot.AllowFrom)
	...
	appendConfigTargets(ch.IRC.Enabled && strings.TrimSpace(ch.IRC.Server) != "", "irc", ch.IRC.AllowFrom)
...
func (t *CronTool) ExecuteJob(ctx context.Context, job *cron.CronJob) string {
	...
	if job.Payload.Command != "" {
		...
		args := map[string]any{
			...
			"__channel": primaryTarget.Channel,
			"__chat_id": primaryTarget.To,
		}
	...
	if t.executor == nil {
		return "Error: no cron executor configured"
	}
